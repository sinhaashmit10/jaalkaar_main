...
const StoleCollection = () => {
  const { products } = useContext(ShopContext)
  const [stoleProducts, setStoleProducts] = useState([])
  const [sortedProducts, setSortedProducts] = useState([])
  const [sortType, setSortType] = useState('relevant')

  // Filter products to include only those with subCategory 'Stole'
  useEffect(() => {
    const filteredProducts = products.filter((item) => item.subCategory === 'Stole')
    setStoleProducts(filteredProducts)
  }, [products])

  // Sort products based on selected sortType
  useEffect(() => {
    let sorted = [...stoleProducts]
    ...
    <div className='flex flex-col gap-2 border-t'>
      ...
      <div className='relative'>
        <div
          ...
          style={{
            backgroundImage: `url(${assets.asset8})`, // Replace with your desired image for Stoles
          }}
        >
          {/* Dark overlay filter */}
          ...
          <div className='relative flex justify-start'>
            <div className='min-w-60 w-full sm:w-3/4 lg:w-1/2 p-16 flex flex-col'>
              ...
              <p className='text-white text-sm sm:text-md md:text-lg'>
                Wrap yourself in elegance with our exquisite Stole collection.
                These versatile and stylish stoles are perfect for any occasion,
                adding a touch of sophistication and charm to your attire.
              </p>
            </div>
          </div>
        </div>
...
export default StoleCollection
