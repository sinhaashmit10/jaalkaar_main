...
const DupattaCollection = () => {
  ...
  const [dupattaProducts, setDupattaProducts] = useState([])
  const [sortedProducts, setSortedProducts] = useState([])
  const [sortType, setSortType] = useState('relevant')

  // Filter products to include only those with subCategory 'Dupatta'
  useEffect(() => {
    const filteredProducts = products.filter(
      (item) => item.subCategory === 'Dupatta'
    )
    setDupattaProducts(filteredProducts)
  }, [products])

  // Sort products based on selected sortType
  useEffect(() => {
    let sorted = [...dupattaProducts]
    ...
    <div className='flex flex-col gap-2 border-t'>
      ...
      <div className='relative'>
        <div
          ...
          style={{
            backgroundImage: `url(${assets.asset15})`, // Replace with your desired image for Dupattas
          }}
        >
          {/* Dark overlay filter */}
          ...
          <div className='relative flex justify-start'>
            <div className='min-w-60 w-full sm:w-3/4 lg:w-1/2 p-16 flex flex-col'>
              <h1 className='text-3xl sm:text-4xl md:text-6xl prata-regular text-[#D4AF37] font-bold mb-4'>
                Dupatta <br />Collection
              </h1>
              <p className='text-white text-sm sm:text-md md:text-lg'>
                Explore our exclusive Dupatta collection – a blend of tradition,
                elegance, and craftsmanship. These dupattas are perfect for
                completing your ensemble with a touch of style and grace.
              </p>
            </div>
          </div>
        </div>
...
export default DupattaCollection
